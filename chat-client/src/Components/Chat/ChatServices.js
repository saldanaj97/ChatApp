...
export const fetchGroupInfo = async (roomId) => {
  const { groupName, userIds } = await axios.get(`http://saldanaj97-chattyio.herokuapp.com/room/${roomId}/roomname`, CONFIG).then((response) => response.data.room);
...
export const fetchUsersInGroup = async (userIds) => {
  const { users } = await axios.post(`http://saldanaj97-chattyio.herokuapp.com/users/usernames`, { userIds: [...userIds] }, CONFIG).then((response) => response.data);
  return users;
};

/* API call to send the message the user has entered */
export const sendMessageInGroup = async (roomId, message) => {
  const response = await axios.post(`http://saldanaj97-chattyio.herokuapp.com/room/${roomId}/message`, { messageText: message }, CONFIG);
  return response;
};

/* API call to get all the messages in a group the user is a part of */
export const retrieveGroupMessages = async (roomId) => {
  const response = await axios.get(`http://saldanaj97-chattyio.herokuapp.com/room/${roomId}`, CONFIG).then((response) => response.data);
  return response;
};
