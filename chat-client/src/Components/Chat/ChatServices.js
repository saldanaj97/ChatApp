...
export const fetchGroupInfo = async (roomId) => {
  const { groupName, userIds } = await axios
    .get(`https://chatapp-production-7d82.up.railway.app/room/${roomId}/roomname`, CONFIG)
...
export const fetchUsersInGroup = async (userIds) => {
  const { users } = await axios
    .post(`https://chatapp-production-7d82.up.railway.app/users/usernames`, { userIds: [...userIds] }, CONFIG)
...
export const sendMessageInGroup = async (roomId, message) => {
  const response = await axios.post(`https://chatapp-production-7d82.up.railway.app/room/${roomId}/message`, { messageText: message }, CONFIG);
  return response;
};

/* API call to get all the messages in a group the user is a part of */
export const retrieveGroupMessages = async (roomId) => {
  const response = await axios.get(`https://chatapp-production-7d82.up.railway.app/room/${roomId}`, CONFIG).then((response) => response.data);
  return response;
};
